...
bool IsWorkspaceValid(
    uint32_t m, uint32_t k, uint32_t rankSize, uint32_t m0, uint32_t commInterval, CostModelConfig const& config,
    CostModelHardwareConfig const& hardware)
{
    uint64_t alignK =
    ...
    uint64_t gatheredSize = ElementsToBytes(
        static_cast<uint64_t>(WORKSPACE_STAGES) * rankSize * commSizeM * alignedK, hardware.communicationElementBits);
    uint64_t gatheredScaleSize = 0;
    if (config.mxScaleGroupSize > 0) {
        uint64_t scaleK = CeilDiv(k, config.mxScaleGroupSize);
        gatheredScaleSize = ElementsToBytes(
            static_cast<uint64_t>(WORKSPACE_STAGES) * rankSize * commSizeM * scaleK, config.mxScaleElementBits);
    }
...
double EstimateAicTileTime(
    uint32_t m0, uint32_t n0, uint32_t k, uint32_t k0, MTECostModel const& mteModel,
    CostModelHardwareConfig const& hardware)
{
...
double EstimateAicWindowTime(
    uint32_t actualCommSizeM, uint32_t n, uint32_t m0, uint32_t n0, uint32_t rankSize, double aicTileTime,
    uint32_t aicCoreNum)
{
...
double EstimateAivWindowTime(
    uint32_t actualCommSizeM, uint32_t k, uint32_t commTileM, uint32_t commBlockM, uint32_t k0, uint32_t commNpuSplit,
    uint32_t commDataSplit, uint32_t rankSize, CostModelConfig const& config, CostModelHardwareConfig const& hardware)
{
...
bool BetterByTieBreak(
    TilingCandidate const& lhs, TilingCandidate const& rhs, uint32_t rankSize, CostModelHardwareType hardwareType,
...
bool IsBetter(
    double cost, TilingCandidate const& candidate, bool hasBest, double bestCost, TilingCandidate const& bestCandidate,
    uint32_t rankSize, CostModelHardwareType hardwareType, CocDataType dataType)
{
            ...
            for (uint32_t commTileM : config.commTileList) {
                ...
                for (auto const& split : config.splitCandidates) {
                    ...
                    for (uint64_t i = 0; i < commLoops; ++i) {
                        ...
                        double aivTime = EstimateAivWindowTime(
                            actualCommSizeM, k, commTileM, commBlockM, k0, split.commNpuSplit, split.commDataSplit,
                            rankSize, config, hardware);
                    ...
                    double rankingCost =
                        (SimulateDoubleBufferPipeline(aivTimes, aicTimes, WORKSPACE_STAGES) +
                    ...
                    if (!IsBetter(
                            rankingCost, candidate, best.IsSuccess(), bestRankingCost, bestCandidate, rankSize,
