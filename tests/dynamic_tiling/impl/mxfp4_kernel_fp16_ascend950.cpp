...
template <template <class, class, class, class, class, class, class, class, class, class> class ConfigAlias>
static void LaunchAscend950Fp4MxAllToAllVGroupedMatmulWithConfig(
    void *stream, uint32_t blockNum, uint64_t fftsAddr,
    KernelParams& kernelParams,
    uint8_t *symmetricPtr, CocTilingParams& cocTiling,
    uint32_t transA, uint32_t transB)
    ...
    uint32_t transA, uint32_t transB)
{
    (void)workSpace;
    if (cocTiling.m0 == 128) {
        LaunchAscend950Fp4MxAllToAllVGroupedMatmulWithConfig<Ascend950Fp4MxAllToAllVGroupedMatmulConfig_M0_128>(
            stream, blockNum, fftsAddr, kernelParams, symmetricPtr, cocTiling, transA, transB);
    } else {
        LaunchAscend950Fp4MxAllToAllVGroupedMatmulWithConfig<Ascend950Fp4MxAllToAllVGroupedMatmulConfig_M0_256>(
            stream, blockNum, fftsAddr, kernelParams, symmetricPtr, cocTiling, transA, transB);
    }
}