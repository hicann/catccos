 ...
 */
#ifndef CATCCOS_DGEMM_KERNEL_ALLGATHER_MATMUL_WITH_RDMA_WRITE_HPP
#define CATCCOS_DGEMM_KERNEL_ALLGATHER_MATMUL_WITH_RDMA_WRITE_HPP
...
public:
    ...
    using RdmaWrite = BlockRdmaWrite_;
    ...
    void operator()<AscendC::AIV>(Params &params)
    {
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx) {
            ...
            if (subcoreIdx == 1 && aicoreIdx < params.rankSize) {
                uint32_t rdmaCoreLoops = params.rankSize;
                uint32_t rdmaAicoreNum = params.rankSize;

                auto rdmaActualShape = MatrixCoord{actualCommShape.row(), actualCommShape.column()};
                ...
                for (uint32_t loopIdx = aicoreIdx; loopIdx < rdmaCoreLoops; loopIdx += rdmaAicoreNum) {
                    ...
                    auto layoutBlockSrc = layoutSymmetric.GetTileLayout(rdmaActualShape);
