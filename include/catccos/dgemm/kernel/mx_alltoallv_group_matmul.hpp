...
template <
    class ProblemShape_, class BlockMmad_, class BlockComm_, class BlockScaleComm_, class BlockMmadScheduler_,
    class BlockCommScheduler_, uint32_t WORKSPACE_STAGES_>
...
public:
    ...
    struct Params {
        ...
        CATLASS_HOST_DEVICE
        Params(
            ProblemShape const& problemShape_, ProblemShape const& scaleProblemShape_, uint32_t commInterval_,
            LayoutTagA tagA_, LayoutTagMxScaleA tagMxScaleA_, LayoutTagB tagB_, GM_ADDR ptrA_, LayoutA const& layoutA_,
            GM_ADDR ptrB_, LayoutB const& layoutB_, GM_ADDR ptrC_, LayoutC const& layoutC_, GM_ADDR ptrMxScaleA_,
            LayoutMxScaleA const& layoutMxScaleA_, GM_ADDR ptrMxScaleB_, LayoutMxScaleB const& layoutMxScaleB_,
            GM_ADDR ptrSymmetric_, GM_ADDR syncMmadFinish_, GM_ADDR syncCommFinish_, GM_ADDR syncScaleMmadFinish_,
            GM_ADDR syncScaleCommFinish_, BlockCommParams const& allToAllVGmmParams_,
            BlockScaleCommParams const& allToAllVGmmScaleParams_)
            : problemShape(problemShape_),
              scaleProblemShape(scaleProblemShape_),
              commInterval(commInterval_),
              tagA(tagA_),
              tagMxScaleA(tagMxScaleA_),
              tagB(tagB_),
              ...
              syncScaleMmadFinish(syncScaleMmadFinish_),
              syncScaleCommFinish(syncScaleCommFinish_),
    ...
    static Params ToUnderlyingArguments(Arguments const& args, uint8_t* workspace = nullptr)
    {
        ...
        MatrixCoord scaleBlockShape{
            args.commBlockShape.row(), CeilDiv<Catlass::MX_SCALE_GROUP_NUM>(args.commBlockShape.column())};
        ...
        return Params(
            problemShape, scaleProblemShape, args.commInterval, tagA, tagMxScaleA, tagB, args.ptrA, layoutA, args.ptrB,
            layoutB, args.ptrC, layoutC, args.ptrMxScaleA, layoutMxScaleA, args.ptrMxScaleB, layoutMxScaleB,
            args.ptrSymmetric, syncMmadFinish, syncCommFinish, syncScaleMmadFinish, syncScaleCommFinish,
            blockCommParams, blockScaleCommParams);
    }
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const& params)
    {
        ...
        auto layoutScaleSymmetric =
            tla::MakeMxScaleLayout<ElementMxScaleA, LayoutTagMxScaleA, false>(mxScalePeerMemM, alignMxScaleK);
        ...
        gmMxScaleA.SetGlobalBuffer(
            reinterpret_cast<__gm__ ElementMxScaleA*>(params.ptrSymmetric) + layoutTagSymmetric.Capacity());
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx) {
            ...
            for (uint32_t localExpertIdx = 0; localExpertIdx < params.problemShape.localExpertNum(); ++localExpertIdx) {
                ...
                gmB.SetGlobalBuffer(
                    params.ptrB + localExpertIdx * layout::Capacity(layoutB) * Catlass::SizeOfBits<ElementB>::value /
                                      Catlass::SizeOfBits<uint8_t>::value);
                AscendC::GlobalTensor<ElementMxScaleB> gmMxScaleB;
                gmMxScaleB.SetGlobalBuffer(
                    params.ptrMxScaleB + localExpertIdx * layout::Capacity(layoutB) / Catlass::MX_SCALE_GROUP_NUM);
                ...
                for (uint32_t srcRankIdx = 0; srcRankIdx < params.problemShape.rankSize(); ++srcRankIdx) {
                    ...
                    for (auto iter = scheduler.Begin(); !iter.End(); iter.Next()) {
                        ...
                        MatrixCoord commOffsetA{
                            layoutSymmetricRow(Catlass::MakeCoord<int>(stageIdx, srcRankIdx, 0)), 0};
                        ...
                        auto tensorBlockB = GetTile(
                            tensorB, tla::MakeCoord(blockOffsetB.row(), blockOffsetB.column()),
                            tla::MakeShape(actualBlockShape.k(), actualBlockShape.n()));
                        auto tensorBlockMxScaleA = GetTile(
                            tensorMxScaleA,
                            tla::MakeCoord(
                                blockOffsetA.row() + commOffsetA.row(),
                                blockOffsetA.column() / Catlass::MX_SCALE_GROUP_NUM),
                            tla::MakeShape(
                                actualBlockShape.m(), CeilDiv<Catlass::MX_SCALE_GROUP_NUM>(actualBlockShape.k())));
                        auto tensorBlockMxScaleB = GetTile(
                            tensorMxScaleB,
                            tla::MakeCoord(blockOffsetB.row() / Catlass::MX_SCALE_GROUP_NUM, blockOffsetB.column()),
                            tla::MakeShape(
                                CeilDiv<Catlass::MX_SCALE_GROUP_NUM>(actualBlockShape.k()), actualBlockShape.n()));
                        auto tensorBlockC = GetTile(
                            tensorC, tla::MakeCoord(blockOffsetC.row(), blockOffsetC.column()),
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(Params const& params)
    {
        ...
        for (int stageIdx = 0; stageIdx < WORKSPACE_STAGES; ++stageIdx) {
            gmSymmetricList[stageIdx].SetGlobalBuffer(
                ptrSymmetric + stageIdx * layout::Capacity(layoutSymmetric) * Catlass::SizeOfBits<ElementA>::value /
                                   Catlass::SizeOfBits<uint8_t>::value);
        }
        auto ptrScaleSymmetric = reinterpret_cast<__gm__ ElementMxScaleA*>(params.ptrSymmetric) +
                                 WORKSPACE_STAGES * layout::Capacity(layoutSymmetric);
        for (int stageIdx = 0; stageIdx < WORKSPACE_STAGES; ++stageIdx) {
            gmScaleSymmetricList[stageIdx].SetGlobalBuffer(
                ptrScaleSymmetric + stageIdx * layout::Capacity(layoutScaleSymmetric));
        }
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx) {
            ...
            for (auto iter = scheduler.Begin(commIdx); !iter.End(); iter.Next()) {
                ...
                blockRemoteCopy(gmBlockSrc, layoutBlockSrc, gmBlockDst, layoutBlockDst, actualBlockShape, dstRankIdx);
            ...
            for (auto iter = scaleScheduler.Begin(commIdx); !iter.End(); iter.Next()) {
                ...
                blockScaleRemoteCopy(
                    gmBlockSrc, layoutBlockSrc, gmBlockDst, layoutBlockDst, actualBlockShape, dstRankIdx);
