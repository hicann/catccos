...
public:
    ...
    struct Params {
        ...
        CATLASS_HOST_DEVICE
        Params(
            ProblemShape const& problemShape_, uint32_t commInterval_, GM_ADDR ptrA_, LayoutA const& layoutA_,
            GM_ADDR ptrB_, LayoutB const& layoutB_, GM_ADDR ptrC_, LayoutC const& layoutC_, GM_ADDR ptrSymmetric_,
            GM_ADDR syncMmadFinish_, GM_ADDR syncCommFinish_, BlockCommParams const& blockCommParams_,
    ...
    static Params ToUnderlyingArguments(Arguments const& args, uint8_t* workspace = nullptr)
    {
        ...
        return Params(
            problemShape, args.commInterval, args.ptrA, layoutA, args.ptrB, layoutB, args.ptrC, layoutC, gmSymmetric,
            syncMmadFinish, syncCommFinish, blockCommParams, blockCommSchedulerParams);
    }
        ...
        for (uint32_t commIdx = 0; commIdx < scheduler.GetCommLoops(); ++commIdx) {
            ...
            for (uint32_t localExpertIdx = 0; localExpertIdx < params.problemShape.localExpertNum(); ++localExpertIdx) {
                ...
                for (auto iter = scheduler.Begin(); !iter.End(); iter.Next()) {
                    ...
                    mmad(gmBlockA, layoutBlockA, gmBlockB, layoutBlockB, gmBlockC, layoutBlockC, actualBlockShape);
                }
            }
