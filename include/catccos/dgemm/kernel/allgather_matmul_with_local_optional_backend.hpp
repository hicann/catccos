 ...
 */
#ifndef CATCCOS_DGEMM_KERNEL_ALLGATHER_MATMUL_LOCAL_MM_H
#define CATCCOS_DGEMM_KERNEL_ALLGATHER_MATMUL_LOCAL_MM_H
...
template <
    class PrologueB_, class BlockMmad_, class BlockComm_, class BlockMmadScheduler_, class BlockCommScheduler_,
    class CommBackend_, uint32_t WORKSPACE_STAGES_>
...
public:
    ...
    struct Params {
        ...
        CATLASS_HOST_DEVICE
        Params(
            GemmCoord const& problemShape_, uint32_t commInterval_, GM_ADDR ptrA_, LayoutA const& layoutA_,
            GM_ADDR ptrB_, LayoutB const& layoutB_, GM_ADDR ptrC_, LayoutC const& layoutC_, GM_ADDR ptrWB_,
            LayoutWB const& layoutWB_, BlockCommParams const& blockCommParams_,
            CommSchedulerParams const& commSchedulerParams_, BackendParams const& backendParams_)
    ...
    static Params ToUnderlyingArguments(Arguments const& args, uint8_t* workspace = nullptr)
    {
        ...
        using LayoutPadding = std::conditional_t<
            std::is_same_v<LayoutB, Catlass::layout::RowMajor>, Catlass::layout::PaddingRowMajor,
        ...
        return Params{
            ...
            blockCommParams,
            commSchedulerParams,
            BackendParams{args.ptrSymmetric}};
    }
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params& params)
    {
        ...
        gmAShmem.SetGlobalBuffer(reinterpret_cast<__gm__ ElementA*>(commBackend.GetPeerMem()));
        ...
        //// Local matmul
        auto localProblemShape =
            Catlass::MakeCoord<uint32_t>(params.problemShape.m(), params.problemShape.n(), params.problemShape.k(), 1);
        ...
        for (uint32_t loopIdx = aicoreIdx; loopIdx < localCoreLoops; loopIdx += aicoreNum) {
            ...
            mmad(
                gmALocal[offsetA], params.layoutA, gmB[offsetB], params.layoutWB, gmC[offsetC], params.layoutC,
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx) {
            ...
            auto actualProblemShape = Catlass::MakeCoord<uint32_t>(
                actualCommSizeM, params.problemShape.n(), params.problemShape.k(), rankSize - 1);
            ...
            for (uint32_t loopIdx = aicoreIdx; loopIdx < coreLoops; loopIdx += aicoreNum) {
                ...
                mmad(
                    gmBlockA, layoutSymmetric, gmBlockB, params.layoutWB, gmBlockC, layoutC,
                    actualBlockShape.GetCoordMNK());
            }
#ifdef ENABLE_TIMER
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(Params& params)
    {
        ...
        BlockComm blockLocalCopy(resource, params.blockCommParams);
        CommBackend commBackend;
        commBackend.Init(params.backendParams);
        uint32_t rankIdx = commBackend.GetRankIdx();
        uint32_t rankSize = commBackend.GetRankSize();
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx) {
            ...
            if (subcoreIdx == 0 && aicoreIdx < commAicoreNum) {
                for (uint32_t commLoopIdx = aicoreIdx; commLoopIdx < commCoreLoops; commLoopIdx += commAicoreNum) {
                    ...
                    uint32_t remoteRankIdx = commBlockCoord.rank() % rankSize;
                    if (remoteRankIdx == rankIdx) {
                        continue;
                    }
                    ...
                    blockLocalCopy(gmBlockSrc, layoutBlockSrc, gmBlockDst, layoutBlockDst, actualCommBlockShape);
                }
            }
