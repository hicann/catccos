...
template <class BlockMmad_, class BlockAllGather_, class BlockUdmaAllGather_, class BlockScheduler_,
...
class Ascend950AllGatherMatmulWithUdma
{
   public:
    ...
    static Params ToUnderlyingArguments(Arguments const &args, uint8_t *workspace = nullptr)
    {
        ...
        return Params(args.problemShape, args.rankIdx, args.rankSize, args.commInterval, tagA, args.ptrA, layoutA,
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx)
        {
            ...
            if (subcoreIdx == 1 && aicoreIdx < params.rankSize)
            {
                ...
                for (uint32_t loopIdx = aicoreIdx; loopIdx < udmaCoreLoops; loopIdx += udmaAicoreNum)
                {
                    ...
                    udmaAllGather(gmBlockSrc, layoutBlockSrc, gmBlockDst, layoutBlockDst, udmaActualShape,
                                  remoteRankIdx);
                }
            }
