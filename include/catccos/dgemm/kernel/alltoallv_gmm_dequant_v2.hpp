...
template <
    class BlockMmad_, class BlockScheduler_, class ElementGroupList_, class RemoteCommBlockEpilogue_,
...
public:
    ...
    struct Params {
        ...
        CATLASS_DEVICE
        Params(
            GemmCoord problemShape_, uint32_t EP_, uint32_t expertPerRank_, uint32_t rank_, uint32_t rankSize_,
            GM_ADDR ptrA_, LayoutA layoutA_, GM_ADDR ptrB_, LayoutB layoutB_, GM_ADDR ptrC_, LayoutC layoutC_,
            GM_ADDR ptrScale_, LayoutScale layoutScale_, GM_ADDR ptrPerTokenScale_, GM_ADDR ptrTokenPerExpert_,
            GM_ADDR ptrCumsumMM_, GM_ADDR symmetricPtr_, RemoteCommParams remoteCommParams_,
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const& params, Catlass::Arch::Resource<ArchTag> resource)
    {
        ...
        for (uint32_t groupIdx = 0; groupIdx < params.expertPerRank; ++groupIdx) {
            ...
            for (uint32_t loopIdx = startLoopIdx; loopIdx < coreLoops; loopIdx += coreNum) {
                ...
                blockMmad(
                    gmA[gmOffsetA], layoutA, gmB[gmOffsetB], layoutB, gmC[gmOffsetC], layoutC, gmScale[gmOffsetS],
                    layoutScale, actualBlockShape);
            }
        ...
        for (int32_t localExpertIdx = 0; localExpertIdx < params.expertPerRank; ++localExpertIdx) {
            ...
            for (int32_t commIdx = 0; commIdx < commLoops; commIdx++) {
                ...
                remoteCommBlockEpilogue(
                    gmBlockSrc, layoutBlockSrc, gmBlockDst, layoutBlockDst, gmBlockPerTokenScale,
                    layoutBlockPerTokenScale, actualSrcBlockShape, dstEpIdx);
