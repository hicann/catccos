...
#ifndef CATCCOS_DGEMM_KERNEL_ASCEND950_GMM_ALLTOALLV_TLA_HPP
#define CATCCOS_DGEMM_KERNEL_ASCEND950_GMM_ALLTOALLV_TLA_HPP
...
>
class Ascend950GMMAlltoallvKernel {
public:
    ...
    friend class GmmAllToAllVEmptyCallBack<Ascend950GMMAlltoallvKernel>;
    ...
    struct WorkspaceInfo {
        ...
        WorkspaceInfo(const Params & params) {
            ...
            ptrcumsumMM = params.ptrWorkspace
                + static_cast<size_t>(params.maxOutputSize) * params.problemShape.n() * sizeof(ElementC);
        }
    };

    CATLASS_DEVICE
    Ascend950GMMAlltoallvKernel()
    {   
    ...
    void operator()<AscendC::AIC>(Params const &params, Catlass::Arch::Resource<ArchTag> resource)
    {
        ...
        for (uint32_t groupIdx = 0; groupIdx < params.expertPerRank; ++groupIdx) {
            ...
            auto tensorA = tla::MakeTensor(gmA[gmGroupOffsetA], layoutA, Catlass::Arch::PositionGM{});
            auto tensorB = tla::MakeTensor(gmB[gmGroupOffsetB], layoutB, Catlass::Arch::PositionGM{});
            auto tensorC = tla::MakeTensor(gmWorkspace[gmGroupOffsetC], layoutC, Catlass::Arch::PositionGM{});
    ...
    void operator()<AscendC::AIV>(Params const &params, Catlass::Arch::Resource<ArchTag> resource)
    {
        ...
        gmSymmetric.SetGlobalBuffer(reinterpret_cast<__gm__ ElementC *>(params.symmetricPtr));
        ...
        for (int32_t localExpertIdx = 0; localExpertIdx < params.expertPerRank; ++localExpertIdx) {
            ...
            for (int32_t commIdx = 0; commIdx < commLoops; commIdx++) { // 每个core负责一个dstEP
                ...
                auto gmBlockSrc = gmWorkspace[params.layoutTagC.GetOffset(blockOffsetSrc)];
                auto gmBlockDst = gmSymmetric[params.layoutTagC.GetOffset(blockOffsetDst)];

                auto actualBlockShape = blockEpilogueScheduler.RemapActualBlockShape(blockOffset, remapperSrc);
                
                auto layoutBlockSrc = params.layoutTagC.GetTileLayout(actualBlockShape);
                auto layoutBlockDst = params.layoutTagC.GetTileLayout(actualBlockShape);
        ...
        if (params.ptrC != nullptr) {
            ...
            if (rowsToCopy > 0) {
                ...
                auto blockSrc = gmSymmetric[params.layoutTagC.GetOffset(blockOffsetSrc)];
                auto blockDst = gmC[params.layoutTagC.GetOffset(blockOffsetSrc)];
