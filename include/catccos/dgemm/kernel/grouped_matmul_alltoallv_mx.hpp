 ...
 */
#ifndef CATCCOS_DGEMM_KERNEL_GROUPED_MATMUL_ALLTOALLV_MX_HPP
#define CATCCOS_DGEMM_KERNEL_GROUPED_MATMUL_ALLTOALLV_MX_HPP
...
template <
    class ProblemShape_, class BlockMmad_, class BlockComm_, class BlockMmadScheduler_, class BlockCommScheduler_,
    uint32_t WORKSPACE_STAGES_>
class GroupedMatmulAllToAllVMx {
public:
    ...
    struct Params {
        ...
        CATLASS_HOST_DEVICE
        Params(
            ProblemShape const& problemShape_, uint32_t commInterval_, LayoutTagComm layoutTagComm_, GM_ADDR ptrA_,
            ...
            GM_ADDR ptrC_, LayoutC const& layoutC_, GM_ADDR ptrSymmetric_, GM_ADDR syncMmadFinish_,
            GM_ADDR syncCommFinish_, BlockCommParams const& blockCommParams_,
    ...
    static Params ToUnderlyingArguments(Arguments const& args, uint8_t* workspace = nullptr)
    {
        ...
        return Params(
            problemShape, args.commInterval, layoutTagC, args.ptrA, layoutA, args.ptrB, layoutB, args.ptrMxScaleA,
            layoutMxScaleA, args.ptrMxScaleB, layoutMxScaleB, args.ptrC, layoutC, gmSymmetric, syncMmadFinish,
            syncCommFinish, blockCommParams, blockCommSchedulerParams);
    }

    CATLASS_DEVICE
    GroupedMatmulAllToAllVMx()
    {
    ...
    CATLASS_DEVICE void operator()<AscendC::AIC>(Params const& params)
    {
        ...
        auto layoutTagSymmetric =
            Catlass::layout::RowMajor::MakeLayout<ElementC>(commShapeM * rankSize, mmadBlockShape.n());
        ...
        for (uint32_t commIdx = 0; commIdx < scheduler.GetCommLoops(); ++commIdx) {
            ...
            for (uint32_t localExpertIdx = 0; localExpertIdx < params.problemShape.localExpertNum(); ++localExpertIdx) {
                ...
                uint32_t localExpertOffsetMxScaleB =
                    localExpertIdx * params.problemShape.k() * params.problemShape.n() / Catlass::MX_SCALE_GROUP_NUM;
                ...
                auto tensorMxScaleB = tla::MakeTensor(
                    gmMxScaleB[localExpertOffsetMxScaleB], params.layoutMxScaleB, Catlass::Arch::PositionGM{});
                ...
                for (auto iter = scheduler.Begin(); !iter.End(); iter.Next()) {
                    ...
                    auto actualBlockShape =
                        scheduler.RemapActualBlockShape(blockOffset.GetCoordMNK(), remapperA, remapperC);

                    auto tensorBlockA = GetTile(
                        tensorA, tla::MakeCoord(blockOffsetA.row(), blockOffsetA.column()),
                    ...
                    auto tensorBlockMxScaleA = GetTile(
                        tensorMxScaleA,
                        tla::MakeCoord(blockOffsetA.row(), blockOffsetA.column() / Catlass::MX_SCALE_GROUP_NUM),
                        tla::MakeShape(
                    ...
                    auto tensorBlockC = GetTile(
                        tensorSymmetric, tla::MakeCoord(blockOffsetC.row() + commOffsetC.row(), blockOffsetC.column()),
    ...
    CATLASS_DEVICE void operator()<AscendC::AIV>(Params const& params)
    {
        ...
        for (int stageIdx = 0; stageIdx < WORKSPACE_STAGES; ++stageIdx) {
            ...
            gmSymmetricList[stageIdx].SetGlobalBuffer(
                ptrSymmetric + srcBlockOffset * layout::Capacity(layoutSymmetric));
        }
        ...
        for (uint32_t commIdx = 0; commIdx < commLoops; ++commIdx) {
            ...
            if (isCommCore && (scheduler.inputSplitBlock[srcEpIdx] > commIdx * blockPerCommInRank)) {
                ...
                for (auto iter = scheduler.Begin(); !iter.End(); iter.Next()) {
                    ...
                    blockRemoteCopy(
                        gmBlockSrc, layoutBlockSrc, gmBlockDst, layoutBlockDst, actualBlockShape, srcRankIdx);
                }
...
#endif // CATCCOS_DGEMM_KERNEL_GROUPED_MATMUL_ALLTOALLV_MX_HPP
