...
struct AllToAllVAllGatherProblemShape {
    ...
    AllToAllVAllGatherProblemShape(
        UnderlyingProblemShape const originProblemShape, uint32_t rankSize, uint32_t rankIdx, uint32_t epSize,
        uint32_t expertNum, GM_ADDR ptrLocalTokensPerExpert, GM_ADDR ptrGlobalTokensPerLocalExpert)
        : originProblemShape_(originProblemShape),
          rankSize_(rankSize),
          rankIdx_(rankIdx),
          epSize_(epSize),
          expertNum_(expertNum),
    ...
    CATLASS_HOST_DEVICE
    uint32_t k() const { return originProblemShape_.k(); }

    CATLASS_HOST_DEVICE
    uint32_t n() const { return originProblemShape_.n(); }

    CATLASS_HOST_DEVICE
    uint32_t rankSize() const { return rankSize_; }

    CATLASS_HOST_DEVICE
    uint32_t rankIdx() const { return rankIdx_; }

    CATLASS_HOST_DEVICE
    uint32_t epSize() const { return epSize_; }

    CATLASS_HOST_DEVICE
    uint32_t tpSize() const { return rankSize_ / epSize_; }

    CATLASS_HOST_DEVICE
    uint32_t localExpertNum() const { return expertNum_ / epSize_; }

    CATLASS_HOST_DEVICE
    uint32_t expertNum() const { return expertNum_; }

    CATLASS_HOST_DEVICE
    uint32_t localTokensPerExpert(uint32_t expertIdx) const { return ptrLocalTokensPerExpert_[expertIdx]; }
    ...
    CATLASS_HOST_DEVICE
    uint32_t globalTokensPerLocalExpert(uint32_t tokenIdx) const { return ptrGlobalTokensPerLocalExpert_[tokenIdx]; }
