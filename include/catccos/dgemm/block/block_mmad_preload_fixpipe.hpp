...
#ifndef CATCCOS_DGEMM_BLOCK_BLOCK_MMAD_PRELOAD_FIXPIPE_HPP
...
public:
    ...
    using TileMmad = Catlass::Gemm::Tile::TileMmad<ArchTag, AType, BType, void>;
    ...
    using CopyL0CToGm = Catlass::Gemm::Tile::CatccosCopyL0CToGm<
        ArchTag, ElementAccumulator, CType, Catlass::Gemm::Tile::ScaleGranularity::PER_CHANNEL>;
    ...
    // Check L1TileShape
    static_assert(
        (L1A_SIZE * STAGES + L1B_SIZE * STAGES + L1S_SIZE) <= ArchTag::L1_SIZE, "L1TileShape exceeding the L1 space!");
    ...
    /// Construct
    CATLASS_DEVICE
    FixpipeBlockMmad(Catlass::Arch::Resource<ArchTag>& resource, uint32_t l1BufAddrStart = 0)
    {
        ...
        GemmCoord const& actualShape, GemmCoord const& actualShapeNext, bool isFirstBlock, bool hasNextBlock)
    {
        ...
        for (uint32_t kLoopIdx = 0; kLoopIdx < kTileCount; kLoopIdx++) {
            ...
            if (shuffleKIdx == lastTileIdx && hasNextBlock) {
                ...
                // load next matrix A tile from GM to L1
                AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1AEventList[l1ListIdNext]);
                auto layoutTileA = layoutA.GetTileLayout(Catlass::MakeCoord(actualShapeNext.m(), kActualNext));
                copyGmToL1A(l1ATensor, gmTileA, layoutAInL1, layoutTileA);
                AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(l1AEventList[l1ListIdNext]);

                // load next matrix B tile from GM to L1
                AscendC::WaitFlag<AscendC::HardEvent::MTE1_MTE2>(l1BEventList[l1ListIdNext]);
                auto layoutTileB = layoutB.GetTileLayout(Catlass::MakeCoord(kActualNext, actualShapeNext.n()));
                copyGmToL1B(l1BTensor, gmTileB, layoutBInL1, layoutTileB);
                AscendC::SetFlag<AscendC::HardEvent::MTE2_MTE1>(l1BEventList[l1ListIdNext]);
            }
        ...
        auto layoutTileS = layoutScale.GetTileLayout(Catlass::MakeCoord(layoutBlock.shape(1)));
        AscendC::WaitFlag<AscendC::HardEvent::FIX_MTE2>(0);
...
#endif // CATCCOS_DGEMM_BLOCK_BLOCK_MMAD_PRELOAD_FIXPIPE_HPP
