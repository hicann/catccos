...
template <
    uint32_t UB_STAGES_, bool IsDynamic_,
...
public:
    ...
    static_assert(
        std::is_same_v<LayoutC, Catlass::layout::RowMajor> && std::is_same_v<LayoutD, Catlass::layout::RowMajor>,
    ...
    static __simd_vf__ inline void CastHighToLow(
        __ubuf__ ElementD* dst, __ubuf__ ElementScale* src, const uint32_t calCount)

    {
        static constexpr AscendC::MicroAPI::CastTrait Fp8LayoutCast = {
            AscendC::MicroAPI::RegLayout::ZERO, AscendC::MicroAPI::SatMode::NO_SAT,
            AscendC::MicroAPI::MaskMergeMode::ZEROING, AscendC::RoundMode::CAST_RINT};
        static constexpr AscendC::MicroAPI::CastTrait Hif8LayoutCast = {
            AscendC::MicroAPI::RegLayout::ZERO, AscendC::MicroAPI::SatMode::NO_SAT,
            AscendC::MicroAPI::MaskMergeMode::ZEROING, AscendC::RoundMode::CAST_ROUND};
        static constexpr AscendC::MicroAPI::CastTrait HalfLayoutCast = {
            AscendC::MicroAPI::RegLayout::ZERO, AscendC::MicroAPI::SatMode::NO_SAT,
            AscendC::MicroAPI::MaskMergeMode::ZEROING, AscendC::RoundMode::CAST_ROUND};
        static constexpr AscendC::MicroAPI::CastTrait FloatLayoutCast = {
            AscendC::MicroAPI::RegLayout::ZERO, AscendC::MicroAPI::SatMode::NO_SAT,
        ...
        if constexpr (
            (std::is_same<ElementD, float8_e4m3_t>::value && std::is_same<ElementScale, half>::value) ||
            (std::is_same<ElementD, float8_e5m2_t>::value &&
             std::is_same<ElementScale, half>::value)) { // half与FP8转换
...

        {
            for (uint16_t i = 0; i < (uint16_t)repeatTimes; ++i) {
                ...
                if constexpr (
                    ...
                    AscendC::MicroAPI::DataCopy<ElementScale, AscendC::MicroAPI::LoadDist::DIST_UNPACK_B16>(
                        srcReg, src + i * srcRepeatSize);
                    ...
                    if constexpr (
                        std::is_same<ElementD, hifloat8_t>::value &&
                        (std::is_same<ElementScale, float>::value || std::is_same<ElementScale, half>::value)) {
                        AscendC::MicroAPI::Cast<ElementD, ElementScale, Hif8LayoutCast>(dstReg, srcReg, preg);
                    }
                    if constexpr (
                        (std::is_same<ElementD, half>::value || std::is_same<ElementD, bfloat16_t>::value) &&

                        std::is_same<ElementScale, float>::value) {
                        AscendC::MicroAPI::Cast<ElementD, ElementScale, HalfLayoutCast>(dstReg, srcReg, preg);
                    }
                    if constexpr (
                        (std::is_same<ElementD, float8_e4m3_t>::value ||
                         std::is_same<ElementD, float8_e5m2_t>::value) &&
                ...
                if constexpr (std::is_same<ElementD, hifloat8_t>::value && std::is_same<ElementScale, half>::value) {
                    AscendC::MicroAPI::DataCopy<ElementD, AscendC::MicroAPI::StoreDist::DIST_PACK_B16>(
                        dst + i * dstRepeatSize, dstReg, preg);
                }
                if constexpr (
                    (std::is_same<ElementD, float8_e4m3_t>::value || std::is_same<ElementD, float8_e5m2_t>::value) &&
                    std::is_same<ElementScale, half>::value) {
                    AscendC::MicroAPI::DataCopy<ElementD, AscendC::MicroAPI::StoreDist::DIST_PACK4_B32>(
                        dst + i * dstRepeatSize, dstReg, preg);
                }
                if constexpr (
                    (std::is_same<ElementD, float8_e4m3_t>::value || std::is_same<ElementD, float8_e5m2_t>::value ||
                ...
                if constexpr (
                    (std::is_same<ElementD, half>::value || std::is_same<ElementD, bfloat16_t>::value) &&
                    std::is_same<ElementScale, float>::value) {
                    AscendC::MicroAPI::DataCopy<ElementD, AscendC::MicroAPI::StoreDist::DIST_PACK_B32>(
                        dst + i * dstRepeatSize, dstReg, preg);
                }
            }
        }
