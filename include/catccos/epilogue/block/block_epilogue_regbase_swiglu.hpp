...
__simd_vf__ inline void SwigluBf16RegBaseVf(
    __ubuf__ bfloat16_t* dst, __ubuf__ bfloat16_t* firstSrc, __ubuf__ bfloat16_t* secondSrc, uint16_t m, uint32_t h,
    uint32_t srcStride, uint32_t dstStride, uint16_t repeatTimes)
{
    ...
    for (uint16_t row = 0; row < m; ++row) {
        ...
        for (uint16_t i = 0; i < repeatTimes; ++i) {
            ...
            AscendC::Reg::LoadAlign<bfloat16_t, AscendC::Reg::LoadDist::DIST_UNPACK_B16>(
                firstBf16, firstSrc + srcOffset);
            AscendC::Reg::LoadAlign<bfloat16_t, AscendC::Reg::LoadDist::DIST_UNPACK_B16>(
                secondBf16, secondSrc + srcOffset);
            ...
            AscendC::Reg::StoreAlign<bfloat16_t, AscendC::Reg::StoreDist::DIST_PACK_B32>(
                dst + dstOffset, dstBf16, mask);
        }
    }
}

__aicore__ inline void SwigluRegBase(
    __ubuf__ bfloat16_t* dst, __ubuf__ bfloat16_t* firstSrc, __ubuf__ bfloat16_t* secondSrc, uint16_t m, uint32_t h,
...
template <
    uint32_t UB_STAGES_, uint32_t TILE_M_, class CType_, class DType_, class TileCopy_, class EpilogueTileSwizzle_>
class BlockEpilogue<
    Catccos::Epilogue::EpilogueAscend950RegBaseSwiglu<UB_STAGES_, TILE_M_>, CType_, DType_, TileCopy_,
...
public:
    ...
    CATLASS_DEVICE
    static void ComputeFromUb(
        AscendC::LocalTensor<ElementD> dstTensor, AscendC::LocalTensor<ElementC> firstTensor,
        AscendC::LocalTensor<ElementC> secondTensor, uint16_t m, uint32_t h, uint32_t srcStride, uint32_t dstStride)
    {
        static_assert(
            AscendC::IsSameType<ElementC, bfloat16_t>::value && AscendC::IsSameType<ElementD, bfloat16_t>::value,
