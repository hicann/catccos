...
class CommBlock<
    AtlasA2CommToShareMem<UB_STAGES_, IsDynamic_>, SrcType_, DstType_, BlockShape_, TileRemoteCopy_, TileSwizzle_,
    GemmRemapper_> {
public:
    // Type aliases
        ...
        uint32_t const& globalLoopIdx, uint32_t const& rankIdx)
    {
        ...
        MatrixCoord actualGemmBlockShape =
            params.gemmRemapper.GetActualBlockShape(remapOutputBlockCoordMNK).GetCoordMN();
        ...
        for (uint32_t innerLoopIdx = 0; innerLoopIdx < tileLoops; innerLoopIdx++) {
            ...
            tileRemoteCopy(
                gmSubblockS, layoutSubblockS, gmSubblockC, layoutSubblockC, actualTileShape, ubSList[ubListId],
