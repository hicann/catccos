...
struct BlockCommSchedulerAllToAllVAllGather {
    ...
    BlockCommSchedulerAllToAllVAllGather(
        ProblemShape const& problemShape_, uint32_t commShapeM_, MatrixCoord const& blockShape_)
    ...
    struct Iter {
        ...
        CATLASS_DEVICE
        bool End() { return (AscendC::GetSubBlockIdx() == 1) || (taskIdx >= coreLoops); }
    };
    ...
    CATLASS_DEVICE
    const RemapperSrc GetRemapperSrc(uint32_t commIdx) { return {this, commIdx}; }
