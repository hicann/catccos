...
public:
    ...
    CATLASS_HOST_DEVICE
    Index const& local() const { return this->At(LOCAL_RANK_INDEX); }

    CATLASS_HOST_DEVICE
    Index& local() { return this->At(LOCAL_RANK_INDEX); }

    CATLASS_HOST_DEVICE
    Index const& remote() const { return this->At(REMOTE_RANK_INDEX); }

    CATLASS_HOST_DEVICE
    Index& remote() { return this->At(REMOTE_RANK_INDEX); }

    CATLASS_HOST_DEVICE
    auto GetMatrixCoord() const { return this->GetCoordByAxis<ROW_INDEX, COLUMN_INDEX>(); }

    CATLASS_HOST_DEVICE
    auto GetLocalCoord() const { return this->GetCoordByAxis<ROW_INDEX, COLUMN_INDEX, LOCAL_RANK_INDEX>(); }

    CATLASS_HOST_DEVICE
    auto GetRemoteCoord() const { return this->GetCoordByAxis<ROW_INDEX, COLUMN_INDEX, REMOTE_RANK_INDEX>(); }
};
