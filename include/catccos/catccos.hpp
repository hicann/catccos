...
struct MinTraits {
    CATLASS_HOST_DEVICE static constexpr auto Apply(T const& lhs, T const& rhs) { return (lhs < rhs) ? lhs : rhs; }
};
...
struct ClipSubTraits {
    CATLASS_HOST_DEVICE static constexpr auto Apply(T const& lhs, T const& rhs)
    {
...
CATLASS_HOST_DEVICE LongIndex Numel(Catlass::Coord<RANK, Index, LongIndex> const& coord)
{
    LongIndex product = 1;
...
template <typename Index, typename LongIndex, int RANK>
CATLASS_HOST_DEVICE LongIndex
Dot(Catlass::Coord<RANK, Index> const& coord, Catlass::Coord<RANK, LongIndex> const& stride, LongIndex accumulator = {})
{
    for (int i = 0; i < RANK; ++i) {
...
template <class T>
CATLASS_HOST_DEVICE constexpr auto Min(T const& lhs, T const& rhs)
{
    return detail::MinTraits<T>::Apply(lhs, rhs);
}

template <typename Index, int RANK>
CATLASS_HOST_DEVICE constexpr auto Min(Catlass::Coord<RANK, Index> const& lhs, Catlass::Coord<RANK, Index> const& rhs)
{
...
template <class T>
CATLASS_HOST_DEVICE constexpr auto Max(T const& lhs, T const& rhs)
{
    return detail::MaxTraits<T>::Apply(lhs, rhs);
}

template <typename Index, int RANK>
CATLASS_HOST_DEVICE constexpr auto Max(Catlass::Coord<RANK, Index> const& lhs, Catlass::Coord<RANK, Index> const& rhs)
{
    Catlass::Coord<RANK, Index> result;
    for (int i = 0; i < RANK; ++i) {
...
template <class T>
CATLASS_HOST_DEVICE constexpr auto ClipSub(T const& lhs, T const& rhs)
{
...
struct MatrixSwizzle {
    CATLASS_DEVICE static Catlass::MatrixCoord GetCoord(Catlass::MatrixCoord const& gridShape, uint32_t loopIdx)
    {
...
struct CommSwizzle {
    CATLASS_DEVICE static DistMatrixCoord GetCoord(
        DistMatrixCoord const& gridShape, Catlass::MatrixCoord const& coreSplit, uint32_t loopIdx)
    {
        constexpr uint32_t ROW_DIM = SWIZZLE_DIRECTION;
        constexpr uint32_t COL_DIM = 1 - SWIZZLE_DIRECTION;
        ...
        uint32_t groupIdx = loopIdx / groupSize;
        uint32_t groupOffset = loopIdx - groupIdx * groupSize;
        uint32_t inGroupRows =
            IS_DETERMINISTIC ? swizzleOffset : Min(swizzleOffset, flattenGridShape[ROW_DIM] - groupIdx * swizzleOffset);
...
CATLASS_HOST_DEVICE constexpr int64_t Capacity(Layout const& layout)
{
...
template <class Type, bool PADDING>
struct PaddingHelper {
    ...
    using LayoutPadding = std::conditional_t<
        std::is_same_v<Layout, Catlass::layout::RowMajor>, Catlass::layout::PaddingRowMajor,
    ...
    using GlobalPadding = std::conditional_t<
        PADDING, Catlass::Gemm::Kernel::PaddingMatrixBlockND<ArchTag, Element, Layout, LayoutPadding, COMPUTE_LENGTH>,
        void>;
...
CATLASS_DEVICE void SetTimerBuffer(__gm__ uint8_t* timerBuffer) { g_timerBufferAddr = timerBuffer; }

CATLASS_DEVICE __gm__ uint8_t* GetTimerBuffer() { return g_timerBufferAddr; }
