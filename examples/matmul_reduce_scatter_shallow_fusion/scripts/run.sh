...
if (( $# >= 1 && $# <= 3 )); then
    ...
    if (( $# == 3 )); then
        ...
        output_root=$(mktemp -d "${TMPDIR:-/tmp}/matmul_reduce_scatter_shallow_fusion.XXXXXX")
    fi
...
if (( n > 32768 )); then
    echo "ReduceScatter requires N <= 32768 (64 KiB communication buffer)" >&2; exit 1
fi
if (( m > 1073741823 / 2 / ranks / n )); then
    echo "ReduceScatter requires 2 * ranks * M * N < 1 GiB" >&2; exit 1
fi
...
binary=$(realpath "$5/matmul_reduce_scatter_shallow_fusion")
...
python "$utils/gen_double_golden_data_mmrs.py" mmrs 1 "$ranks" "$kernel_m" "$n" "$k" 0 "$trans_b" "$data_dir" "${seed_args[@]}" > gen-data.log 2>&1
...
python "$utils/verify_result.py" "$data_dir/output.bin" "$data_dir/golden_fp32.bin" 1 \
    "$kernel_m" "$n" "$k" --golden_low "$data_dir/golden_aclnn.bin" | tee verify.log
