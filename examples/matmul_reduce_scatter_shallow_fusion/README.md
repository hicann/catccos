...
bash examples/matmul_reduce_scatter_shallow_fusion/scripts/build.sh /tmp/catccos-build
run_root=$(mktemp -d /tmp/matmul_reduce_scatter_shallow_fusion.XXXXXX)
CATCCOS_SEED=2026 bash examples/matmul_reduce_scatter_shallow_fusion/scripts/run.sh 0,2 3 257 255 \
    /tmp/catccos-build/bin "$run_root/case"
```

`0,2` 是两张设备的示例，请替换为本机可用且支持 SHMEM 互访的设备 ID。构建目录首次使用时应为空或不存在，后续可由本分支的四个样例共享；切换源码目录、CANN/SHMEM 安装或 A5 架构后应另用新目录，避免沿用旧 CMake 缓存。本例编译目标为 A2（dav-c220）。运行时保留完整 `bin` 目录，其中包含可执行文件及其依赖的 `libmatmul_reduce_scatter_shallow_fusion_impl.so`，并在运行终端加载上述环境。

CSV 批量回归（读取本样例的 [scripts/test_shapes.csv](scripts/test_shapes.csv)）：

```bash
bash examples/matmul_reduce_scatter_shallow_fusion/scripts/run.sh 0,2 /tmp/catccos-build/bin
```
...
```text
matmul_reduce_scatter_shallow_fusion rank_size rank_id ip_port M N K data_path [device_id_list [transB]]
```

B 文件在 transB=0/1 时分别按 K×N / N×K 存储。脚本参数 M 表示每个 rank 的输出行数；传给生成器和二进制的 M 为该值乘以 rank 数。输入为 `rank_<id>_a.bin`、`rank_<id>_b.bin`，各 rank 按行偏移写入同一个 `output.bin`，与现有 ReduceScatter 的文件约定一致。

`run.sh` 直接调用已有的 `gen_double_golden_data_mmrs.py mmrs` 生成 CPU FP32 与 ACLNN FP16 双标杆，启动各 rank 执行一次，然后调用公共 `verify_result.py --golden_low`。精度公式和阈值沿用公共实现。单个 case 的所有数据位于其输出目录的 `output/` 下，生成日志为 `gen-data.log`，运行日志为 `rank-<id>.log`，校验日志为 `verify.log`；运行或校验失败返回非零退出码。

自定义模式支持 2–16 卡；二进制参数 M 是全局行数，必须能被卡数整除，脚本会完成换算。脚本中的设备 ID 必须互不重复并按升序排列（当前 CANN 的可见设备列表要求）。所有模式均要求 M/N/K 为正整数；transA 固定为 0，transB 默认为 0。

本例要求 `N <= 32768`：当前全同步归约路径必须将一整行 FP16 输出放入 64 KiB 通信缓冲。按脚本中的每卡 M 计算，共享数据区要求 `2 × ranks × M × N < 1073741824` 字节；直接调用二进制时，其 M 已是全局行数，不再乘 ranks。
