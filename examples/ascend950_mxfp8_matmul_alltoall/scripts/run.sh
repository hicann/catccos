...
cd ${PROJECT_ROOT}/examples/ascend950_mxfp8_matmul_alltoall/

DATA_DIR=`realpath ./out`
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/ascend950_mxfp8_matmul_alltoall
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    # Validate N is divisible by RANK_SIZE (AllToAll along N)
    if [ $((N % RANK_SIZE)) -ne 0 ]; then
        echo "[ERROR] N=${N} must be divisible by RANK_SIZE=${RANK_SIZE}"
        continue
    fi

    # Generate golden data
    rm -rf ./out/*.bin
    python3 ${SCRIPT_DIR}/gen_data_slice_n.py ${OUT_DTYPE} ${RANK_SIZE} ${M} ${N} ${K} ${transA} ${transB} ${DATA_DIR}
    ...
    for (( idx = 0; idx < ${RANK_SIZE}; idx = idx + 1 )); do
        ...
        python3 ${UTILS_PATH}/verify_result.py ${DATA_DIR}/output_${idx}.bin ${DATA_DIR}/golden_${idx}.bin ${OUT_DTYPE} ${M} ${N} ${K}
    done
done
