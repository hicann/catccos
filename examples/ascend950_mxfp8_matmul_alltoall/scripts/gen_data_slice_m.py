...
def gen_matmul_alltoall_data():
    """
    MXFP8 MatMul + AlltoAll data generation:
    ...
    - Golden for rank j: assembled D_j of shape (M, N) in fp32
    """
    parser = argparse.ArgumentParser(description="Generate MXFP8 MatmulAllToAll test data")
    parser.add_argument('out_dtype', type=int, nargs='?', default=1, help='Output dtype (1=FP16, for compatibility)')
    parser.add_argument('rank_size', type=int, help='Number of ranks')
    parser.add_argument('m', type=int, help='Matrix M dimension (must be divisible by rank_size)')
    ...
    if M % rank_size != 0:
        raise ValueError(f"M ({M}) must be divisible by rank_size ({rank_size})")
    ...
    for i in range(rank_size):
        ...
        # Reshape scale to match kernel layout: [M, scaleK//2, 2]
    ...
    # Generate shared B (E4M3 + scale)
    b_fp8, b_scale, b_fp32 = gen_data_fp8_e4m3(K, N, 0)  # axis=0 (row-wise)

    # Reshape scale to match kernel layout: [scaleK//2, 2, N]
    ...
    # Write shared B to each rank's file (same content)
    for i in range(rank_size):
    ...
    # Compute per-rank MatMul results (fp32 golden)
    ...
    for j in range(rank_size):
        ...
        golden_j = torch.cat(chunks, dim=0)  # shape: (M, N) fp32

        tensor_to_file(golden_j, os.path.join(data_dir, f"golden_{j}.bin"))

    print(
        f"Generated MXFP8 MatMul+AllToAll data: M={M}, N={N}, K={K}, "
        f"rank_size={rank_size}, chunk={chunk}"
    )
