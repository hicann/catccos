 ...
 */
#include "ascend950_fp4_mx_alltoallv_grouped_matmul_host.h"
#include "ascend950_fp4_mx_alltoallv_grouped_matmul_device.h"
...
using Config = Ascend950Fp4MxAllToAllVGroupedMatmulConfig_M0_128<ElementA, LayoutA, ElementB, LayoutB, ElementMxScaleA, LayoutMxScaleA, ElementMxScaleB, LayoutMxScaleB, ElementC, LayoutC>;
...
int main(int argc, char **argv)
{
    ...
    cocTiling.n0 = 256;
    cocTiling.k0 = 256;
    cocTiling.commTileM = 256;
    cocTiling.commInterval = 2;
    cocTiling.commNpuSplit = 1;
    cocTiling.commDataSplit = 16;
    cocTiling.commBlockM = 256;
    ...
    auto op = OperatorRegistry::Instance().CreateOperator("Ascend950Fp4MxAllToAllVGroupedMatmul");
    if (!op) {
        std::cout << "Operator Ascend950Fp4MxAllToAllVGroupedMatmul not found!" << std::endl;
        return -1;
    }
    ...
    std::cout << "Before calling FP4_MX_ATAV_GMM kernel " << std::endl;
    uint64_t fftsAddr = shmemx_get_ffts_config();
    for (int i = 0; i < 1; i++) {
        deviceOp.Run(stream, blockNum, fftsAddr);
    }
    ACL_CHECK(aclrtSynchronizeStream(stream));
    std::cout << "After calling FP4_MX_ATAV_GMM kernel " << std::endl;
