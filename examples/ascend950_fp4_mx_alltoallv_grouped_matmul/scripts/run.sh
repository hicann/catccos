...
cd ${PROJECT_ROOT}/examples/ascend950_fp4_mx_alltoallv_grouped_matmul/

DATA_DIR=`realpath ./out`
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/ascend950_fp4_mx_alltoallv_grouped_matmul
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    python3 ${UTILS_PATH}/gen_data_fp4_mx_alltoallv_gmm.py "a5fp4mxatavgmm" 1 ${RANK_SIZE} ${M} ${N} ${K} ${transA} ${transB} ${DATA_DIR} --expert ${EXPERT_NUM} --ep ${EP_SIZE}

    # Set necessary parameters
    IPPORT="tcp://127.0.0.1:27032"

    # Start Process
    echo "Running ascend950_fp4_mx_alltoallv_grouped_matmul"
