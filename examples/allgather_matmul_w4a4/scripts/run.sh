...
cd ${PROJECT_ROOT}/examples/allgather_matmul_w4a4/

DATA_DIR=`realpath ./out`
mkdir -p ./out
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/allgather_matmul_w4a4

tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    echo "Processing test case: M=${M}, K=${K}, N=${N}"

    rm -rf ./out/*.bin
    python3 ${UTILS_PATH}/gen_allgather_matmul_w4a4_data.py "agmmw4a4" 27 ${RANK_SIZE} ${M} ${N} ${K} 0 0 ${DATA_DIR}
    ...
    for (( idx =0; idx < ${RANK_SIZE}; idx = idx + 1 )); do
        python3 ${UTILS_PATH}/verify_result.py \
            ${DATA_DIR}/output_rank${idx}.bin \
            ${DATA_DIR}/golden_rank${idx}.bin \
            27 ${M} ${N} ${K} | awk '{ print } /PASS/ { pass = 1 } END { exit pass ? 0 : 1 }' || exit 1
    done
done
