 ...
 */
#include "ascend950_alltoallv_grouped_matmul_device.h"
#include "ascend950_alltoallv_grouped_matmul_host.h"
...
struct Options {
    static constexpr auto helper = "Usage: ascend950_alltoallv_grouped_matmul rank_size rank_id ip m n k ep expert_num device_list\n";
...
int main(int argc, char **argv)
{
    ...
    cocTiling.epSize = options.epSize; cocTiling.expertNum = options.expertNum;

    std::cout << "[TEST] rank_size:" << rankSize << " rank_id:" << rankId << " ip:" << options.ipPort << "\n";
    ...
    if (!op) { std::cout << "Operator Ascend950AllToAllVGroupedMatmul not found!" << std::endl; return -1; }
