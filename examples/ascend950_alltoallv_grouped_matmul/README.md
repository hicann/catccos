...
1. **编译项目**  
   进入示例目录并执行编译脚本：
   ```bash
   cd examples/ascend950_alltoallv_grouped_matmul
   bash scripts/build.sh
   ```

2. **运行Ascend950-AllToAllV-GroupedMatMul示例程序**  
   在示例目录下执行运行脚本：
   ```bash
   ...
   - **参数说明**：
     ...
     - `expert_num`: 指定专家数，未显示指定时默认值为8。
        ...
        示例：使用第6和第7个NPU设备运行2卡Ascend950-AllToAllV-GroupedMatMul, 专家并行数为2, 专家数为8：
       ```bash
       # 修改运行参数配置时可能需要同步shape文件
       bash scripts/run.sh 6,7 2 4
       ```
