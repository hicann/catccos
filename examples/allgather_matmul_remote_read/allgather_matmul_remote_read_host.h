 ...
 */
#ifndef ALLGATHER_MATMUL_REMOTE_READ_HOST_H
#define ALLGATHER_MATMUL_REMOTE_READ_HOST_H

#include "catlass/catlass.hpp"
#include "operator_registry.h"

class AllGatherMatmulRemoteReadOperator : public CatccosOperator {
public:
    ...
    CocCommType GetActualKernelType(const CocTilingParams &cocTiling) override {
        return CocCommType::ALLGATHER_MATMUL_REMOTE_READ;
    }

    bool CheckCocTilingParams(uint32_t rankSize, const CocTilingParams& cocTiling) override {
        uint64_t alignK = Catlass::BYTE_PER_FRACTAL / sizeof(__fp16);
        uint64_t alignedK = (static_cast<uint64_t>(cocTiling.k) + alignK - 1) / alignK * alignK;
        uint64_t aStageSize = static_cast<uint64_t>(cocTiling.m) * alignedK * INPUT_DTYPE;
        uint64_t commSizeM = static_cast<uint64_t>(cocTiling.commInterval) * cocTiling.m0;
        uint64_t gatheredSize = static_cast<uint64_t>(WORKSPACE_STAGES) * rankSize * commSizeM * alignedK * INPUT_DTYPE;
        if (aStageSize + gatheredSize > static_cast<uint64_t>(SHMEM_BUFF_BYTES)) {
            return false;
        }
        return true;
    }
...
// 注册这个算子
REGISTER_OPERATOR("AllGatherMatmulRemoteRead", AllGatherMatmulRemoteReadOperator);

#endif // ALLGATHER_MATMUL_REMOTE_READ_HOST_H
