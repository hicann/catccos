...
cd ${PROJECT_ROOT}/examples/allgather_matmul_remote_read/

DATA_DIR=`realpath ./out`
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/allgather_matmul_remote_read
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf ./out/*.bin
    python3 ${UTILS_PATH}/gen_data.py "agmmrr" 1 ${RANK_SIZE} ${M} ${N} ${K} 0 0 ${DATA_DIR}
