...
cd ${PROJECT_ROOT}/examples/dispatch_ffn_combine/
EXEC_BIN=${PROJECT_ROOT}/build/bin/dispatch_ffn_combine
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf output/*.bin
    python3 ${UTILS_PATH}/gen_data_moe.py "dispatch_ffn_combine" ${DATA_TYPE} ${RANK_SIZE} ${M} ${N} ${K} --top_k 4 --expert $EXPERT_NUM --ep $EP_SIZE
