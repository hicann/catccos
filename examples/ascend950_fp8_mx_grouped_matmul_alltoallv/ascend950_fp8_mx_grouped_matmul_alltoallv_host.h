 ...
 */
#ifndef ASCEND950_FP8_MX_GROUPED_MATMUL_ALLTOALLV_HOST_H
#define ASCEND950_FP8_MX_GROUPED_MATMUL_ALLTOALLV_HOST_H

#include "operator_registry.h"
#include "catlass/detail/alignment.hpp"

class Ascend950Fp8MxGroupedMatmulAllToAllVOperator : public CatccosOperator {
public:
        ...
        KernelParams& params, const CocTilingParams& cocTiling, uint32_t rankId, std::string dataFile) override
    {
        ...
        size_t bSize = static_cast<size_t>(cocTiling.k) * cocTiling.n * sizeof(int8_t) * localExpertNum;
        size_t cSize = static_cast<size_t>(cocTiling.m) * cocTiling.n * sizeof(fp16_t) * cocTiling.rankSize;
        size_t localTokensPerExpertSize = static_cast<size_t>(cocTiling.expertNum) * sizeof(int32_t);
        size_t globalTokensPerLocalExpertSize =
            static_cast<size_t>(cocTiling.rankSize) * localExpertNum * sizeof(int32_t);
        ...
        if (dataFile != "") {
            ...
            ReadFileOrThrow(dataFile + "/a_scale_" + std::to_string(rankId) + ".bin", aMxScaleHost, sizeMxScaleA);
            ...
            ACL_CHECK(aclrtMemcpy(
                aMxScaleDevice, sizeMxScaleA, matrixAMxScale.data(), sizeMxScaleA, ACL_MEMCPY_HOST_TO_DEVICE));
        }
        ...
        if (dataFile != "") {
            ...
            ReadFileOrThrow(dataFile + "/b_scale_" + std::to_string(rankId) + ".bin", bMxScaleHost, sizeMxScaleB);
            ...
            ACL_CHECK(aclrtMemcpy(
                bMxScaleDevice, sizeMxScaleB, matrixBMxScale.data(), sizeMxScaleB, ACL_MEMCPY_HOST_TO_DEVICE));
        }
        ...
        ACL_CHECK(
            aclrtMalloc((void**)(&localTokensPerExpertDevice), localTokensPerExpertSize, ACL_MEM_MALLOC_HUGE_FIRST));
        uint8_t* localTokensPerExpertHost;
        uint8_t* globalTokensPerLocalExpertDevice;
        ACL_CHECK(aclrtMalloc(
            (void**)(&globalTokensPerLocalExpertDevice), globalTokensPerLocalExpertSize, ACL_MEM_MALLOC_HUGE_FIRST));
        ...
        if (dataFile != "") {
            ...
            std::vector<uint32_t> matrixG(
                gSize / sizeof(uint32_t), cocTiling.m / cocTiling.expertNum * cocTiling.rankSize / cocTiling.epSize);
            ACL_CHECK(
                aclrtMemcpy(globalTokensPerLocalExpertDevice, gSize, matrixG.data(), gSize, ACL_MEMCPY_HOST_TO_DEVICE));
        }
        ...
        params.SetKernelParams(
            aDevice, bDevice, cDevice, aMxScaleDevice, bMxScaleDevice, localTokensPerExpertDevice,
            globalTokensPerLocalExpertDevice);
    ...
    {
        return CocCommType::ASCEND950_FP8_MX_GROUPED_MATMUL_ALLTOALLV;
    }
...
REGISTER_OPERATOR("Ascend950Fp8MxGroupedMatmulAllToAllV", Ascend950Fp8MxGroupedMatmulAllToAllVOperator);

#endif // ASCEND950_FP8_MX_GROUPED_MATMUL_ALLTOALLV_HOST_H
