...
1. **编译项目**  
   进入示例目录并执行编译脚本：
   ```bash
   cd examples/ascend950_fp8_mx_grouped_matmul_alltoallv
   bash scripts/build.sh
   ```

2. **运行Ascend950-FP8-MX-GroupedMatMul-AllToAllV示例程序**  
   进入示例目录并执行运行脚本：
   ```bash
   cd examples/ascend950_fp8_mx_grouped_matmul_alltoallv
   ...
   - **参数说明**：
     ...
     - `expert_num`: 指定专家数，未显示指定时默认值为8。
        ...
        示例：使用第6和第7个NPU设备运行2卡Ascend950-FP8-MX-GroupedMatMul-AllToAllV, 专家并行数为2, 专家数为8：
       ```bash
