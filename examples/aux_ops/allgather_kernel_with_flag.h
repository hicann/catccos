...
public:
    ...
    struct Params {
        ...
        CATLASS_DEVICE
        Params(uint32_t elementNum_, GM_ADDR srcPtr_, GM_ADDR dstPtr_, GM_ADDR symmetricPtr_)
    ...
    void operator()(Params& params, Catlass::Arch::Resource<ArchTag> resource)
    {
        ...
        auto layoutUb =
            Catlass::layout::RowMajor{1, elementNum, RoundUp<Catlass::BYTE_PER_C0 / sizeof(Element)>(elementNum)};
            ...
            if (dstRank != rankIdx) {
                ...
                for (int32_t checkIdx = 0; checkIdx < elementNum; checkIdx += intPer512) {
                    __gm__ int32_t* sync_check =
                        reinterpret_cast<__gm__ int32_t*>(params.symmetricPtr) + dstRank * elementNum + checkIdx;
