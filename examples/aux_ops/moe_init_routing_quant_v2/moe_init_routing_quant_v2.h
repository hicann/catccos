...
#ifndef MOE_INIT_ROUTING_QUANT_V2_H
#define MOE_INIT_ROUTING_QUANT_V2_H
    ...
    const MoeInitRoutingQuantV2TilingData* tilingData, uint64_t tilingKey)
{
    ...
    if (tilingKey == 20000) { // quant full load
        ...
        op.Init(
            x, expertIdx, scale, offset, expandedX, expandedRowIdx, expertTokensCountOrCumsum, workspace, tilingData,
            &sortPipe);
        op.Process();
        sortPipe.Destroy();
        return;
        ...
        op.Init(
            x, expertIdx, expandedX, expandedRowIdx, expertTokensCountOrCumsum, scale, dynamicQuantScale, workspace,
            tilingData, &sortPipe);
        op.Process();
        sortPipe.Destroy();
        return;
    ...
    if (tilingKey == 10000 || tilingKey == 10010 || tilingKey == 11000 || tilingKey == 11010) { // No drop scenario
        ...
        if (tilingKey == 10100 || tilingKey == 10110) {
            ...
            srcToDstAndGatherOp.Init(
                x, scale, expandedRowIdx, expandedX, dynamicQuantScale, workspace, tilingData, &srcToDstGatherPipe);
    ...
    if (tilingKey == 10000 || tilingKey == 10010 || tilingKey == 10100 || tilingKey == 10110) {
        ...
        gatherDynamicQuantOp.Init(
            x, scale, expandedRowIdx, expandedX, dynamicQuantScale, workspace, tilingData, &gatherPipe);
        gatherDynamicQuantOp.Process();
        gatherPipe.Destroy();
    }
}

#endif // MOE_INIT_ROUTING_QUANT_V2_H
