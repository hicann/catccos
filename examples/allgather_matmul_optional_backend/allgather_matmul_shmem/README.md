...
1. **编译项目**
   进入示例目录并执行编译脚本：
   ```bash
   cd examples/allgather_matmul_optional_backend/allgather_matmul_shmem
...
该示例仅构建独立的 `allgather_matmul_shmem` C++ 可执行程序，不接入 PyTorch 扩展，也不加入 `run_all_examples.sh` 的执行列表。
