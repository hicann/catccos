...
void RunAgmmDeviceOp(aclrtStream stream, uint32_t blockDim, uint64_t fftsAddr, const CocTilingParams& cocTiling,
                     const Catlass::GemmCoord& problemShape, uint8_t* aPtr, uint8_t* bPtr, uint8_t* cPtr,
                     uint8_t* workspaceDevice, uint8_t* gmSymmetric, const Catlass::MatrixCoord& commCoreSplit,
                     const Catlass::MatrixCoord& commBlockShape, const Catlass::MatrixCoord& commTileShape, int rankId,
                     int rankSize)
{
...
void catccos_allgather_matmul_wrapper(uint32_t blockDim, aclrtStream stream, uint64_t fftsAddr, uint8_t* aPtr,
                                      uint8_t* bPtr, uint8_t* cPtr, uint8_t* gmSymmetric, uint32_t m, uint32_t n,
                                      uint32_t k, int rankId, int rankSize)
{
    ...
    if (workSpaceSize > 0)
    {
        auto ret = aclrtMalloc(reinterpret_cast<void**>(&workspaceDevice), workSpaceSize, ACL_MEM_MALLOC_HUGE_FIRST);
        if (ret != ACL_SUCCESS)
        {
            std::cerr << "Allocate workspace failed, size=" << workSpaceSize << ", ret=" << ret << std::endl;
            return;
        }
    }
    ...
    if (isNeedPaddingB)
    {
        RunAgmmDeviceOp<ConfigPadding::Device>(stream, blockDim, fftsAddr, cocTiling, problemShape, aPtr, bPtr, cPtr,
                                               workspaceDevice, gmSymmetric, commCoreSplit, commBlockShape,
                                               commTileShape, rankId, rankSize);
    }
    else
    {
        RunAgmmDeviceOp<ConfigNoPadding::Device>(stream, blockDim, fftsAddr, cocTiling, problemShape, aPtr, bPtr, cPtr,
                                                 workspaceDevice, gmSymmetric, commCoreSplit, commBlockShape,
                                                 commTileShape, rankId, rankSize);
    }
