...
void RunAgmmDeviceOp(
    aclrtStream stream, uint32_t blockDim, uint64_t fftsAddr, const CocTilingParams& cocTiling,
    const Catlass::GemmCoord& problemShape, uint8_t* aPtr, uint8_t* bPtr, uint8_t* cPtr, uint8_t* workspaceDevice,
    uint8_t* gmSymmetric, const Catlass::MatrixCoord& commCoreSplit, const Catlass::MatrixCoord& commBlockShape,
    const Catlass::MatrixCoord& commTileShape, int rankId, int rankSize)
{
    ...
    uint8_t* gmSymmetric, uint32_t m, uint32_t n, uint32_t k, int rankId, int rankSize)
{
    ...
    if (isNeedPaddingB) {
        RunAgmmDeviceOp<ConfigPadding::Device>(
            stream, blockDim, fftsAddr, cocTiling, problemShape, aPtr, bPtr, cPtr, workspaceDevice, gmSymmetric,
            commCoreSplit, commBlockShape, commTileShape, rankId, rankSize);
    } else {
