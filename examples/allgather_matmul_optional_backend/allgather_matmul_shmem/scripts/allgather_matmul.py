...
def worker(args):
    ...
    try:
        ...
        has_pending_exception = sys.exc_info()[0] is not None
        try:
            torch_npu.npu.synchronize()
            finalize_status = torch.ops.catccos.finalize()
            if finalize_status != 0:
                message = (
                    f"Rank {rank_id}: torch.ops.catccos.finalize failed "
                    f"with status {finalize_status}"
                )
            ...
            if has_pending_exception:
                print(
                    f"Rank {rank_id}: finalize raised while another exception was pending: "
                    f"{finalize_error}",
                    file=sys.stderr,
                )
            else:
