...
if [ ! -f "${SHMEM_ENV}" ]; then
    echo "[ERROR] SHMEM environment ${SHMEM_ENV} not found. Please run scripts/build.sh first."
    exit 1
fi
source "${SHMEM_ENV}" || {
...
cd "${PROJECT_ROOT}/examples/allgather_matmul_optional_backend/allgather_matmul_shmem"

mkdir -p ./out
DATA_DIR=$(realpath ./out)
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN="${PROJECT_ROOT}/build/bin/allgather_matmul_shmem"
if [ ! -x "${EXEC_BIN}" ]; then
...
while IFS=',' read -r M K N; do
    ...
    if [ "${rank_status}" -ne 0 ]; then
        echo "[ERROR] allgather_matmul_shmem failed for M=${M}, K=${K}, N=${N}"
        exit 1
    fi
