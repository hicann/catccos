 ...
 */
#include "ascend950_allgather_matmul_shmem_device.h"
#include "ascend950_allgather_matmul_shmem_host.h"
...
struct Options {
    static constexpr auto HELPER = "Usage: ascend950_allgather_matmul_shmem "
...
int main(int argc, char** argv)
{
    ...
    if (status != ACLSHMEM_SUCCESS) {
        std::cerr << "aclshmemx_init_attr failed, rankId=" << rankId << ", status=" << status << std::endl;
        ACL_CHECK(aclrtDestroyStream(stream));
        ACL_CHECK(aclrtResetDevice(deviceId));
        ACL_CHECK(aclFinalize());
        return 1;
    ...
    auto op = OperatorRegistry::Instance().CreateOperator("Ascend950AllGatherMatmulShmem");
    if (!op) {
        std::cerr << "Operator Ascend950AllGatherMatmulShmem not found!" << std::endl;
        shmem_finalize();
        ACL_CHECK(aclrtDestroyStream(stream));
        ACL_CHECK(aclrtResetDevice(deviceId));
        ACL_CHECK(aclFinalize());
    ...
    if (symmPtr == nullptr) {
        std::cerr << "shmem_malloc failed, rankId=" << rankId << ", size=" << SHMEM_BUFF_BYTES << std::endl;
    ...
    if (status != ACLSHMEM_SUCCESS) {
        std::cerr << "shmem_finalize failed, rankId=" << rankId << ", status=" << status << std::endl;
    }
    ACL_CHECK(aclrtDestroyStream(stream));
    ACL_CHECK(aclrtResetDevice(deviceId));
    ACL_CHECK(aclFinalize());
