...
source "${PROJECT_ROOT}/examples/utils/setup.sh" -soc_type Ascend950 || {
    echo "[ERROR] Running A5 setup.sh failed."
    exit 1
}
...
cmake -B "$BUILD_DIR" -S "$SOURCE_DIR" \
    -DCATCCOS_BUILD_TESTS=OFF \
    -DNPU_MODULE="${ASCEND950_NPU_MODULE:-Ascend950}" \
    -DASCEND_COMPUTE_UNIT="${ASCEND950_COMPUTE_UNIT:-ascend950}"
cmake --build "$BUILD_DIR" --target ascend950_allgather_matmul_hccl -j
