...
if [ "$RANK_SIZE" -lt 1 ] || [ "$RANK_SIZE" -gt 16 ]; then
    echo "Rank size is illegal"
    exit 1
fi

cd "${PROJECT_ROOT}/examples/allgather_matmul_optional_backend/ascend950_allgather_matmul_hccl/"

mkdir -p ./out
DATA_DIR=$(realpath ./out)
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=./build/ascend950_allgather_matmul_hccl
