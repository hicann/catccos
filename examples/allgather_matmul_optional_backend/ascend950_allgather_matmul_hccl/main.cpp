...
int Sample(void* arg)
{
    ...
    auto a = aclCreateTensor(
        aDims.data(), aDims.size(), ACL_FLOAT16, aStride.data(), 0, ACL_FORMAT_ND, aDims.data(), aDims.size(), aDevice);
    auto b = aclCreateTensor(
        bDims.data(), bDims.size(), ACL_FLOAT16, bStride.data(), 0, ACL_FORMAT_ND, bDims.data(), bDims.size(), bDevice);
    auto c = aclCreateTensor(
        cDims.data(), cDims.size(), ACL_FLOAT16, cStride.data(), 0, ACL_FORMAT_ND, cDims.data(), cDims.size(), cDevice);
    ...
    if (auto ret = aclnnAllGatherMatmulHcclGetWorkspaceSize(
    ...
    if (auto ret = aclrtSynchronizeStream(stream); ret != ACL_SUCCESS) {
        ...
        // process exits after the rank threads join.
        cleanup(false);
        return ret;
    }
