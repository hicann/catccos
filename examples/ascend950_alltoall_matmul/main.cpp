 ...
 */
#include "ascend950_alltoall_matmul_device.h"
#include "ascend950_alltoall_matmul_host.h"
...
using Config = Ascend950AllToAllMatmulConfig_M0_128<ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC>;
...
int main(int argc, char **argv)
{
    ...
    auto op = OperatorRegistry::Instance().CreateOperator("Ascend950AllToAllMatmul");
    if (!op)
    {
        std::cout << "Operator Ascend950AllToAllMatmul not found!" << std::endl;
        return -1;
    }
    ...
    std::cout << "Before calling AllToAll_MM kernel " << std::endl;
    for (int i = 0; i < 1; i++)
    {
        deviceOp.Run(stream, BLOCK_NUM, fftsAddr);
    }
    ACL_CHECK(aclrtSynchronizeStream(stream));
    std::cout << "After calling AllToAll_MM kernel " << std::endl;

    // Write result for all ranks (each rank has its own output)
