...
if (( $# >= 1 && $# <= 3 )); then
    ...
    [[ ${header%$'\r'} == "M,K,N,transB,seed" ]] || { echo "Invalid CSV header" >&2; exit 1; }
    if (( $# == 3 )); then
        output_root=$(realpath -m "$3")
        mkdir -p "$(dirname "$output_root")"
        mkdir "$output_root"
    else
        output_root=$(mktemp -d "${TMPDIR:-/tmp}/allgather_matmul_shallow_fusion.XXXXXX")
    fi
    echo "CSV results: $output_root"
    case_id=0
    while IFS=',' read -r m k n trans_b seed || [[ -n $m ]]; do
        seed=${seed%$'\r'}
        ...
        CATCCOS_SEED=$seed bash "$script_dir/run.sh" "$1" "$m" "$n" "$k" \
            "$bin_dir" "$output_root/case-$case_id" "$trans_b"
...
if (( $# < 6 || $# > 7 )); then
    ...
    echo "Usage: $0 device_id_list M N K bin_dir new_output_dir [transB]" >&2
    exit 1
fi
...
if (( k > 49152 )); then
    echo "AllGather requires K <= 49152 (96 KiB communication buffer)" >&2; exit 1
fi
if (( m > 1073741823 / 2 / ranks / (((k + 255) / 256) * 256) )); then
    echo "AllGather requires 2 * ranks * M * round_up(K, 256) < 1 GiB" >&2; exit 1
fi
...
if (( ranks & (ranks - 1) )); then
    echo "This example requires a power-of-two rank count" >&2
    exit 1
fi
binary=$(realpath "$5/allgather_matmul_shallow_fusion")
...
python "$utils/gen_data.py" agmm 1 "$ranks" "$m" "$n" "$k" 0 "$trans_b" "$data_dir" --double-golden "${seed_args[@]}" > gen-data.log 2>&1
...
for ((rank = 0; rank < ranks; ++rank)); do
    timeout --kill-after=5s "${CATCCOS_TIMEOUT:-120}" "$binary" "$ranks" "$rank" \
        ...
        "$local_devices" "$trans_b" > "rank-$rank.log" 2>&1 &
    pids+=("$!")
done
...
status=0
for ((rank = 0; rank < ranks; ++rank)); do
    if python "$utils/verify_result.py" "$data_dir/rank_${rank}_output.bin" "$data_dir/golden.bin" 1 \
        "$((m * ranks))" "$n" "$k" --golden_low "$data_dir/golden_low.bin" > "verify-$rank.log" 2>&1; then
        echo "Rank $rank: PASS"
    else
        cat "verify-$rank.log"
        status=1
