...
public:
    ...
    void WriteResultFile(const KernelParams &params, const CocTilingParams &cocTiling,
        uint32_t rankId, std::string dataFile) override {
        size_t cSize = static_cast<size_t>(cocTiling.m) * cocTiling.topK * cocTiling.n * sizeof(__fp16) * cocTiling.rankSize;
    ...
    size_t GetWorkspaceSize(const CocTilingParams &cocTiling) override {
        ...
        size_t workspaceSize = static_cast<size_t>(maxOutputSize) * cocTiling.k * sizeof(half) + EP * EP * expertPerRank * sizeof(int32_t);
