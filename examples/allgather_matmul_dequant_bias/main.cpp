 ...
 */
#include "allgather_matmul_dequant_bias_host.h"
#include "allgather_matmul_dequant_bias_device.h"
...
struct Options {
    static constexpr auto HELPER =
       "Usage: allgather_matmul_dequant_bias rank_size rank_id ip_port m n k [device_id_list]\n";
...
int main(int argc, char **argv)
{
    ...
    auto op = OperatorRegistry::Instance().CreateOperator("AllGatherMatmulDequantBias");
    if (!op) {
        std::cout << "Operator AllGatherMatmulDequantBias not found!" << std::endl;
        return -1;
    }
    ...
    for (int i = 0; i < 1; i++) {
        ...
        AllGatherMatmulDequantBias<ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC, ElementScale, LayoutScale>
            <<<BLOCK_NUM, nullptr, stream>>>(fftsAddr, aPtr, bPtr, cPtr, biasPtr, scalePtr, gmSymmetric, cocTiling);
    }
    ACL_CHECK(aclrtSynchronizeStream(stream));
