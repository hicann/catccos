...
cd ${PROJECT_ROOT}/examples/allgather_matmul_dequant_bias/

DATA_DIR=`realpath ./out`
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/allgather_matmul_dequant_bias
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf ./out/*.bin
    python3 ${UTILS_PATH}/gen_allgather_quant_data.py "agmmdqbs" 1 ${RANK_SIZE} ${M} ${N} ${K} 0 0 ${DATA_DIR}
    ...
    # Verify output
    for (( idx =0; idx < ${RANK_SIZE}; idx = idx + 1 )); do
        python3 ${UTILS_PATH}/verify_result.py ${DATA_DIR}/output_rank${idx}.bin ${DATA_DIR}/golden_rank${idx}.bin 1 ${M} ${N} ${K}
    done
done
