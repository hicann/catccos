...
DATA_TYPE=27
EXPERT_NUM=EXPERT_NUM=${2:-4}
TOP_K=4
EP_SIZE=$RANK_SIZE
EP_PRE_RANK=$((EXPERT_NUM / $EP_SIZE))

cd ${PROJECT_ROOT}/examples/dispatch_gmm_dequant_swiglu/
EXEC_BIN=${PROJECT_ROOT}/build/bin/dispatch_gmm_dequant_swiglu
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf output/*.bin
    python3 ${UTILS_PATH}/gen_data_dispatch_gmm_dequant_swiglu.py "dispatch_gmm_dequant_swiglu" ${DATA_TYPE} ${RANK_SIZE} ${M} ${N} ${K} --top_k ${TOP_K} --expert $EXPERT_NUM --ep $EP_SIZE
