...
public:
        ...
        KernelParams& params, const CocTilingParams& cocTiling, uint32_t rankId, std::string dataFile) override
    {
        ...
        if (dataFile != "") {
            ...
            ReadFileOrThrow(dataFile + x1_filename, x1Host, x1Size);
            ACL_CHECK(aclrtMemcpy(x1Device, x1Size, x1Host, x1Size, ACL_MEMCPY_HOST_TO_DEVICE));
        } else {
            std::vector<int8_t> matrixX1(static_cast<size_t>(cocTiling.m) * cocTiling.k, 1);
        ...
        if (dataFile != "") {
            ...
            ReadFileOrThrow(dataFile + x2_filename, x2Host, x2Size);
            ACL_CHECK(aclrtMemcpy(x2Device, x2Size, x2Host, x2Size, ACL_MEMCPY_HOST_TO_DEVICE));
        } else {
            std::vector<int8_t> matrixX2(static_cast<size_t>(cocTiling.k) * cocTiling.n, 1);
        ...
        if (dataFile != "") {
            ...
            ReadFileOrThrow(dataFile + "/scale_x1_gm.bin", scaleX1Host, scaleX1Size);
            ...
            ACL_CHECK(
                aclrtMemcpy(scaleX1Device, scaleX1Size, matrixScaleX1.data(), scaleX1Size, ACL_MEMCPY_HOST_TO_DEVICE));
        }
        ...
        if (dataFile != "") {
            ...
            ReadFileOrThrow(dataFile + "/scale_x2_gm.bin", scaleX2Host, scaleX2Size);
            ...
            ACL_CHECK(
                aclrtMemcpy(scaleX2Device, scaleX2Size, matrixScaleX2.data(), scaleX2Size, ACL_MEMCPY_HOST_TO_DEVICE));
        }
    ...
    size_t GetWorkspaceSize(const CocTilingParams& cocTiling) override
    {
        ...
        return static_cast<size_t>(WORKSPACE_STAGES) * blockPerCommInRank * cocTiling.m0 * cocTiling.n0 *
               cocTiling.rankSize * sizeof(int32_t);
    }
