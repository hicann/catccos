...
cd ${PROJECT_ROOT}/examples/dispatch_gmm_swiglu/
EXEC_BIN=${PROJECT_ROOT}/build/bin/dispatch_gmm_swiglu
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf output/*.bin
    python3 ${UTILS_PATH}/gen_data_moe.py "dispatch_gmm_swiglu" ${DATA_TYPE} ${RANK_SIZE} ${M} ${N} ${K} --top_k 4 --expert $EXPERT_NUM --ep $EP_SIZE
    ...
    # Verify output
    for (( idx =0; idx < ${RANK_SIZE}; idx = idx + 1 )); do
        python3 ${UTILS_PATH}/verify_result.py ./output/output_${idx}.bin ./output/out_swiglu_${idx}.bin $DATA_TYPE ${M} ${N} ${K} &
    done
