...
if (( $# >= 1 && $# <= 3 )); then
    ...
    if (( $# == 3 )); then
        ...
        output_root=$(mktemp -d "${TMPDIR:-/tmp}/matmul_allreduce_shallow_fusion.XXXXXX")
    fi
...
if (( m > 1073741823 / 2 / n )); then
    echo "AllReduce requires 2 * M * N < 1 GiB" >&2; exit 1
fi
...
binary=$(realpath "$5/matmul_allreduce_shallow_fusion")
...
python "$utils/gen_data.py" mmar 1 "$ranks" "$m" "$n" "$k" 0 "$trans_b" "$data_dir" --double-golden "${seed_args[@]}" > gen-data.log 2>&1
...
for ((rank = 0; rank < ranks; ++rank)); do
    if python "$utils/verify_result.py" "$data_dir/rank_${rank}_output.bin" "$data_dir/golden.bin" 1 \
        "$m" "$n" "$k" --golden_low "$data_dir/golden_low.bin" > "verify-$rank.log" 2>&1; then
        echo "Rank $rank: PASS"
    else
        cat "verify-$rank.log"
        status=1
