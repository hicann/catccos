 ...
 */
#include "ascend950_matmul_reduce_scatter_device.h"
#include "ascend950_matmul_reduce_scatter_host.h"
...
using Config = Ascend950MatmulReduceScatterConfig_M0_128<ElementA, LayoutA, ElementB, LayoutB, ElementD, LayoutD>;
using DeviceOp = Config::Device;

struct Options {
    static constexpr auto HELPER = "Usage: matmul_reduce_scatter rank_size rank_id ip_port m n k [device_id_list]\n";
...
int main(int argc, char** argv)
{
    ...
    auto op = OperatorRegistry::Instance().CreateOperator("Ascend950MatmulReduceScatter");
    if (!op) {
        std::cout << "Operator Ascend950MatmulReduceScatter not found!" << std::endl;
        return -1;
    }
