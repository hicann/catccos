...
cd ${PROJECT_ROOT}/examples/ascend950_matmul_reduce_scatter/
DATA_DIR=`realpath ./output`
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/ascend950_matmul_reduce_scatter
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf ./output/*.bin
    python3 ${UTILS_PATH}/gen_data.py "mmrs" 1 ${RANK_SIZE} ${M} ${N} ${K} 0 0 ${DATA_DIR}
