...
cd ${PROJECT_ROOT}/examples/ascend950_allgather_matmul_udma/

DATA_DIR=`realpath ./out`
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/ascend950_allgather_matmul_udma
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf ./out/*.bin
    python3 ${UTILS_PATH}/gen_data_allgather_matmul.py "agmm" 1 ${RANK_SIZE} ${M} ${N} ${K} 0 0 ${DATA_DIR}
