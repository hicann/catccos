 ...
 */
#include "ascend950_allgather_matmul_udma_host.h"
#include "ascend950_allgather_matmul_udma_device.h"
...
using Config = Ascend950AllGatherMatmulUdmaConfig_M0_128<ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC>;
using DeviceOp = Config::Device;

struct Options {
    static constexpr auto HELPER =
       "Usage: ascend950_allgather_matmul_udma rank_size rank_id ip_port m n k [device_id_list]\n";
...
int main(int argc, char **argv)
{
    ...
    // Use UDMA data operation engine
    attributes.option_attr.data_op_engine_type = ACLSHMEM_DATA_OP_UDMA;
    status = aclshmemx_init_attr(ACLSHMEMX_INIT_WITH_DEFAULT, &attributes);

    auto op = OperatorRegistry::Instance().CreateOperator("Ascend950AllGatherMatmulUdma");
    if (!op) {
        std::cout << "Operator Ascend950AllGatherMatmulUdma not found!" << std::endl;
        return -1;
    }
    ...
    std::cout << "Before calling AG_MM_UDMA kernel " << std::endl;
    uint64_t fftsAddr = shmemx_get_ffts_config();
    for (int i = 0; i < 1; i++) {
        deviceOp.Run(stream, blockNum, fftsAddr);
    }
    ACL_CHECK(aclrtSynchronizeStream(stream));
    std::cout << "After calling AG_MM_UDMA kernel " << std::endl;
