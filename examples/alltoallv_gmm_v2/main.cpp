...
struct Options {
    static constexpr auto helper = "Usage: alltoall_grouped_matmul m n k ep_size expert_num device_list\n";
    ...
    int Parse(int argc, char **argv)
    {
        ...
        if (argc > INDEX_MAX || argc <= DEVICE_LIST_INDEX) {
            printf(helper);
            return -1;
        }
        ...
        expertPerRank = std::atoi(argv[EXPERT_PER_RANK_INDEX]);
...
int main(int argc, char **argv)
{
    ...
    uint8_t *tokenPerExpertDevice = kernelParams.customPtrs[0];
    ACL_CHECK(aclrtSynchronizeStream(stream));

    for (int i = 0; i < 1; i++) {
        uint64_t fftsAddr = shmemx_get_ffts_config();
        AllToAllVGMMV2<ElementA, LayoutA, ElementB, LayoutB, ElementC, LayoutC>
            <<<BLOCK_NUM, nullptr, stream>>>(
                fftsAddr,
                aPtr,
                bPtr,
                cPtr,
                tokenPerExpertDevice,
                workspaceDevice,
                symmetricPtr,
                cocTiling
            );
