...
cd ${PROJECT_ROOT}/examples/ascend950_grouped_matmul_alltoallv/
EXEC_BIN=${PROJECT_ROOT}/build/bin/ascend950_grouped_matmul_alltoallv
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    rm -rf output/*.bin
    python3 ${UTILS_PATH}/gen_data_gmm_alltoallv.py 5 1 ${RANK_SIZE} ${M} ${N} ${K} 0 0 --expert $EXPERT_NUM --ep $EP_SIZE
