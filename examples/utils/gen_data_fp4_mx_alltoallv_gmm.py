...
def gen_data_a_fp4_e2m1(matrix: torch.Tensor, row, col, axis, transpose):
    ...
    return quantized_matrix_uint8, scale_matrix

def gen_data_b_fp4_e2m1(matrix: torch.Tensor, local_expert_num, row, col, axis, transpose):
    ...
    quantized_tensors = torch.zeros(local_expert_num, row, ((col + 1) // 2), 
                                   dtype=torch.uint8)
    ...
    for i in range(local_expert_num):
        ...
        quantized_tensors[i] = quantized_matrix_uint8
        scale_tensors[i] = scale_matrix

    return quantized_tensors, scale_tensors

def gen_dequantized_matrix_a_fp4_e2m1(matrix: torch.Tensor, rank_size, row, col, axis):
...
def gen_dequantized_matrix_b_fp4_e2m1(matrix: torch.Tensor, rank_size, local_expert_num, row, col, axis):
...
def generate_data(args: argparse.Namespace) -> None:
    ...
    matrix_a_dequantized_origin_world = gen_dequantized_matrix_a_fp4_e2m1(matrix_a_origin_world, rank_size, m, k, 1)
    matrix_b_dequantized_origin_world = gen_dequantized_matrix_b_fp4_e2m1(matrix_b_origin_world, rank_size, local_expert_num, k, n, 0)
    ...
    for rank_idx in range(rank_size):
        a_fp8, a_scale = gen_data_a_fp4_e2m1(matrix_a_world[rank_idx], m, k, 1, args.trans_a)
        b_fp8, b_scale = gen_data_b_fp4_e2m1(matrix_b_world[rank_idx], local_expert_num, k, n, 0, args.trans_b)
