...
def gen_golden_data():
    ...
    for i in range(args.rank_size):
        a_uint8, a_scale, a_fp32 = gen_data_fp4_e2m1(M, K, 1, args.transA)
        ...
        a_gm_path = os.path.join(data_dir, f"input_a_{i}.bin")
        a_scale_gm_path = os.path.join(data_dir, f"input_a_scale_{i}.bin")
        a_np = torch.tensor(a_uint8.flatten().untyped_storage(), dtype=torch.int8)
        ...
        matrix_a_list.append(a_fp32)

    allgathered_a = torch.cat(matrix_a_list, dim=0)

    for i in range(args.rank_size):
        b_uint8, b_scale, b_fp32 = gen_data_fp4_e2m1(K, N, 0, args.transB)
        ...
        matrix_c = torch.matmul(allgathered_a, b_fp32)

        b_gm_path = os.path.join(data_dir, f"input_b_{i}.bin")
        b_scale_gm_path = os.path.join(data_dir, f"input_b_scale_{i}.bin")
        b_np = torch.tensor(b_uint8.flatten().untyped_storage(), dtype=torch.int8)
