...
def generate_data(args: argparse.Namespace) -> None:
    ...
    tensor_to_file(global_tokens_per_expert, os.path.join(data_dir, f"global_tokens_per_expert_matrix.bin"))
    
    for rank_idx in range(rank_size):
        tensor_to_file(matrix_a_world[rank_idx], os.path.join(data_dir, f"a_gm_{rank_idx}.bin"))
        tensor_to_file(matrix_b_world[rank_idx], os.path.join(data_dir, f"b_gm_{rank_idx}.bin"))
