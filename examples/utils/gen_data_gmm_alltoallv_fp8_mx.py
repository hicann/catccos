...
def gen_data_fp8_e4m3_all_same(row, col, axis, num):
    matrix = num * torch.ones((row, col), dtype=torch.float32)
...
def gen_data_fp8_e4m3_as_row_idx(row, col, axis):
    row_indices = torch.arange(row, dtype=torch.float32).reshape(-1, 1) + 1
    matrix = row_indices.expand(-1, col)
...
def gen_data_fp8_e4m3_as_col_idx(row, col, axis):
    col_indices = torch.arange(col, dtype=torch.float32).reshape(1, -1)
    matrix = torch.ones((row, 1), dtype=torch.float32) * col_indices
...
def generate_data(args: argparse.Namespace) -> None:
    ...
    matrix_a_origin_world_fp8 = torch.zeros(ep_size, m, k, dtype=torch.float8_e4m3fn)
    matrix_b_origin_world_fp8 = torch.zeros(ep_size, local_expert_num, k, n, dtype=torch.float8_e4m3fn)
    ...
    for rank in range(ep_size):
        ...
        matrix_a_origin_world_fp8[rank] = a_fp8
        ...
        for expert in range(local_expert_num):
            ...
            matrix_b_origin_world_fp8[rank, expert] = b_fp8
    ...
    matrix_a_world = matrix_a_origin_world_fp8 if not trans_a else matrix_a_origin_world_fp8.mT
    matrix_a_scale_world = matrix_a_scale_origin_world if not trans_a else matrix_a_scale_origin_world.mT
    matrix_b_world = matrix_b_origin_world_fp8 if not trans_b else matrix_b_origin_world_fp8.mT
