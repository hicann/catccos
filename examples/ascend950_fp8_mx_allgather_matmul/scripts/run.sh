...
cd ${PROJECT_ROOT}/examples/ascend950_fp8_mx_allgather_matmul/

DATA_DIR=`realpath ./out`
echo "DATA_DIR: $DATA_DIR"
EXEC_BIN=${PROJECT_ROOT}/build/bin/ascend950_fp8_mx_allgather_matmul
...
tail -n +2 "$CSV_FILE" | while IFS=',' read -r M K N; do
    ...
    python3 ${UTILS_PATH}/gen_data_fp8_mx_allgather_matmul.py "a5fp8mxagmm" 1 ${RANK_SIZE} ${M} ${N} ${K} ${transA} ${transB} ${DATA_DIR}

    # Set necessary parameters
    IPPORT="tcp://127.0.0.1:27032"

    # Start Process
    echo "Running ascend950_fp8_mx_allgather_matmul"
