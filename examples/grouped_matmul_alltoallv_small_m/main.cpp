...
#include "grouped_matmul_alltoallv_small_m_device.h"
#include "grouped_matmul_alltoallv_small_m_host.h"

struct Options {
    uint32_t rankSize, rankId, groups = 2;
    ...
    int Parse(int argc, char** argv)
    {
        ...
        if (argc > 8) {
            ...
            for (uint32_t i = 0; i < rankSize; ++i)
                devices.push_back(i);
        if (argc > 9)
            groups = std::stoul(argv[9]);
        if (argc > 10)
            tiling.transB = std::stoul(argv[10]);
        tiling.rankSize = tiling.epSize = rankSize;
        tiling.expertNum = groups * rankSize;
        return rankId < rankSize && devices.size() == rankSize && !dataPath.empty() && groups ? 0 : -1;
    }
};
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        std::cerr << e.what() << "\nUsage: " << argv[0]
                  << " rank_size rank_id ip_port m n k data_path [device_id_list "
                     "[local_experts [transB]]]\n";
        return 1;
    }
    auto op = OperatorRegistry::Instance().CreateOperator("GroupedMatmulAlltoAllvSmallM");
    ...
    try {
        ...
        Catccos::Examples::GroupedMatmulAlltoAllvSmallMExample::Launch(
